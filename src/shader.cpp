		...
		if(t_type == COMPUTE_SHADER) {
			...
			if(t_src[0] != '#') {
				t_src = 
					...
					// for testing
					"\n"
					"#define LIGHT_AMBIENT   1.2f\n"
					"#define LIGHT_DIFFUSE   1.25f\n"
					"#define LIGHT_SPECULAR  1.2f\n"
					"#define GAMMA 1.5f\n"

					
					"vec3 vpanic_light(vec3 light_pos, vec4 light_color, float light_radius) {"
						" vec3 item_color = (fragment.texcoord == vec2(0.0f)) ? shape.color.xyz : texture(texture0, fragment.texcoord).rgb;"
						" if(item_color == vec3(0.0f) && shape.color.xyz != vec3(0.0f)) { "
							" item_color = shape.color.xyz;"
						" }"
						" item_color *= light_color.xyz;"

						" vec3 norm = normalize(fragment.normal);"
						" vec3 light_dir = normalize(light_pos-fragment.pos);"
						...
						" float spec = pow(max(dot(normalize(light_dir+view_dir), norm), 0.0f), 64.f);"
						" vec3 specular = LIGHT_SPECULAR*spec*item_color;"

						" float dist = length(light_pos-fragment.pos);"
						" light_radius *= 0.5f;"
