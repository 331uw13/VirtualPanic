		...
		if(flag == VCORE_COMPILE_USER_SHADER) {
			const char src_utils[] = 
				...
				"vec3 compute_light() {\n"
					...
					" vec3 light_color = vec3(lights[0].red/255.f, lights[0].green/255.f, lights[0].blue/255.f);\n"
					" vec3 item_color = shape_color.xyz * light_color;\n"
					...
					"float light_radius = (15.f+lights[0].radius) * 0.5f;\n"
					"float dist = length(light_pos - frag.pos);\n"
					"float att          = smoothstep(light_radius + dist, 0.0f, dist);\n"
					"float att_sample   = smoothstep(light_radius + dist-0.5, 0.05f, dist*1.5);\n"
					"att = att*att_sample;\n"
