...
namespace vpanic {
	...
	void ParticleSystem::init() {
		...
		*/
		...
		glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(Particle)*m_max_count, NULL, GL_STATIC_DRAW);
		...
		glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(Particle), 0);
		glEnableVertexAttribArray(6);
		
		glVertexAttribPointer(7, 4, GL_FLOAT, GL_FALSE, sizeof(Particle), (void*)sizeof(Vec4));
		glEnableVertexAttribArray(7);
		
		glVertexAttribPointer(8, 4, GL_FLOAT, GL_FALSE, sizeof(Particle), (void*)(sizeof(Vec4)*2));
	...
	void ParticleSystem::resize(const uint32_t t_count) {
		...
		// TODO: optimize late, im tired xd
	...
	void ParticleSystem::update(const Shader& t_shader, const float t_delta_time, const Matrix& t_camera_view) {		
		...
		const uint32_t group_size = m_max_count/GROUP_SIZE_X;
